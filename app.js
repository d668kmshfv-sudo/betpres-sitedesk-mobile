(() => {
 ...
 const ARRAYS=["projects","companies","assignments","purchases","defects","controlDays","calendarEvents","workerSheets","betpresTimesheets","thpTimesheets","mobileDiary","materialSamples"];
 ...
 function renderRoute(){
  ...
  if(app.route==="samples")renderSamples();
 ...
 function isDiaryDone(item){return Boolean(item?.completed||item?.completedAt)||/splnen|hotov|vybaven/i.test(String(item?.status||""))}
 ...
 function recentActivities(){
  return [
   ...
   ...currentProjectItems("mobileDiary").filter(item=>!isDiaryDone(item)).map(item=>recordActivity(item,"✎",item.title||"Denný zápis",item.place,item.createdAt))
 ...
 function renderHome(){
  ...
  const date=todayISO(),workers=workersForDate(date),openDefects=currentProjectItems("defects").filter(item=>!isDefectDone(item)),diary=currentProjectItems("mobileDiary").filter(item=>!isDiaryDone(item)&&String(item.createdAt||"").slice(0,10)===date);
 ...
 function renderDiary(){
  ...
  const items=currentProjectItems("mobileDiary").filter(item=>!isDiaryDone(item)&&(!query||[item.title,item.text,item.place].some(value=>String(value||"").toLowerCase().includes(query)))).sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||"")));
  $("diaryList").innerHTML=items.length?items.map(item=>`<article class="list-item diary-task-card"><button class="diary-complete-button" type="button" data-complete-diary="${esc(item.id)}" aria-label="Označiť ako hotové">✓</button><div class="item-main"><strong>${esc(item.title||"Zápis zo stavby")}</strong><span>${esc(item.text||"Bez textu")}</span><span>${esc(item.place||"Bez miesta")}${item.dueDate?` · termín ${formatDate(item.dueDate)}`:""}</span></div><div class="item-side"><strong>${esc(item.priority||"Stredná")}</strong><small>${formatDate(String(item.createdAt||"").slice(0,10))}</small></div></article>`).join(""):`<div class="empty-state">Všetko je hotové. Nový zápis sa zobrazí tu aj v počítači.</div>`;
  qa("[data-complete-diary]",$("diaryList")).forEach(button=>button.onclick=()=>completeDiary(button.dataset.completeDiary))
 }
 function completeDiary(id){
  const item=app.data.mobileDiary.find(row=>row.id===id);if(!item||isDiaryDone(item))return;
  const now=isoNow(),record={...item,completed:true,completedAt:now,status:"Splnené",updatedAt:now};
  upsertRecord("mobileDiary",record,"add-diary");renderDiary();toast("Úloha je hotová a po synchronizácii zmizne aj v počítači.")
 }

 function sampleSubmitter(item){return company(item.companyId)?.name||item.submittedBy||"BETPRES, s.r.o."}
 function renderSamples(){
  const input=$("sampleSearch"),query=String(input?.value||"").trim().toLowerCase();
  const items=currentProjectItems("materialSamples").filter(item=>!query||[item.materialName,item.manufacturer,item.productType,item.location,sampleSubmitter(item)].some(value=>String(value||"").toLowerCase().includes(query))).sort((a,b)=>Number(b.sequence||0)-Number(a.sequence||0));
  $("sampleList").innerHTML=items.length?items.map(item=>`<article class="list-item sample-mobile-card" data-edit-sample="${esc(item.id)}"><span class="item-icon sample-number">${esc(item.sequence||"◇")}</span><div class="item-main"><strong>${esc(item.materialName||"Vzorka bez názvu")}</strong><span>${esc(item.manufacturer||item.productType||"Výrobca neuvedený")}</span><span>${esc(sampleSubmitter(item))}${item.location?` · ${esc(item.location)}`:""}</span></div><div class="item-side"><strong class="sample-status">${esc(item.status||"Rozpracovaná")}</strong><small>${formatDate(item.submittedDate)}</small><button class="defect-edit-button" type="button">Upraviť</button></div></article>`).join(""):`<div class="empty-state">Zatiaľ tu nie je žiadna materiálová vzorka. Stlač +, pridaj technický list a fotografiu.</div>`;
  qa("[data-edit-sample]",$("sampleList")).forEach(card=>card.onclick=()=>openSampleForm(card.dataset.editSample))
 }
 ...
 function openSampleForm(sampleId=""){
  const existing=sampleId?app.data.materialSamples.find(item=>item.id===sampleId&&item.projectId===app.projectId):null,nextSequence=Math.max(0,...currentProjectItems("materialSamples").map(item=>Number(item.sequence)||0))+1;
  showSheet({eyebrow:"SCHVAĽOVANIE MATERIÁLOV",title:existing?`Upraviť vzorku č. ${existing.sequence||""}`:"Nová materiálová vzorka",html:`<form id="sampleFormMobile" class="sheet-form"><div class="form-split"><label>Poradové číslo<input id="sampleSequenceInput" inputmode="numeric" value="${esc(existing?.sequence||nextSequence)}"></label><label>Dátum predloženia<input id="sampleDateInput" type="date" required value="${esc(existing?.submittedDate||todayISO())}"></label></div><label>Názov materiálu<input id="sampleNameInput" required value="${esc(existing?.materialName||"")}" placeholder="napr. minerálna izolácia"></label><div class="form-split"><label>Výrobca<input id="sampleManufacturerInput" value="${esc(existing?.manufacturer||"")}"></label><label>Typ výrobku<input id="sampleTypeInput" value="${esc(existing?.productType||"")}"></label></div><label>Navrhované použitie<textarea id="sampleUseInput" placeholder="Kde a na čo sa materiál použije">${esc(existing?.intendedUse||"")}</textarea></label><div class="form-split"><label>Objekt / miesto<input id="sampleLocationInput" value="${esc(existing?.location||"")}"></label><label>Predložila firma<select id="sampleCompanyInput">${sampleCompanyOptions(existing)}</select></label></div><label>Stav<select id="sampleStatusInput">${["Rozpracovaná","Predložená","Schválená","Zamietnutá"].map(value=>`<option ${value===(existing?.status||"Rozpracovaná")?"selected":""}>${value}</option>`).join("")}</select></label><div class="sample-attachment-grid"><label>📄 Technický list<input id="sampleTechnicalInput" type="file" accept="application/pdf,image/*"></label><label>📷 Fotografia vzorky<input id="samplePhotoInput" type="file" accept="image/*" capture="environment"></label><label>🖼 Fotografia z galérie<input id="sampleGalleryInput" type="file" accept="image/*"></label></div><p id="sampleAttachmentStatus" class="form-note">${existing?.technicalSheet?`Technický list: ${esc(existing.technicalSheet.name||"priložený")}. `:""}${existing?.photo?"Fotografia je priložená.":"Pridaj technický list a fotografiu vzorky."}</p><div id="samplePhotoPreview" class="sample-photo-preview"></div><div class="sheet-actions"><button class="cancel" type="button">Zrušiť</button><button class="submit" type="submit">Uložiť vzorku</button></div></form>`,onReady:()=>{
   let technical=existing?.technicalSheet?clone(existing.technicalSheet):null,photo=existing?.photo?clone(existing.photo):null;
   const renderPreview=()=>{$("samplePhotoPreview").innerHTML=photo?mobilePhotoImage(photo,"sample-edit-thumb"):"";requestAnimationFrame(()=>hydrateMobileCloudPhotos($("samplePhotoPreview")))};renderPreview();
   $("sampleTechnicalInput").onchange=async event=>{const file=event.target.files?.[0];if(!file)return;try{technical=await fileAttachment(file,12*1024*1024);$("sampleAttachmentStatus").textContent=`Technický list: ${technical.name}${photo?" · fotografia priložená":""}`}catch(error){$("sampleAttachmentStatus").textContent=error.message}event.target.value=""};
   const pickPhoto=async event=>{const file=event.target.files?.[0];if(!file)return;try{photo=await imageData(file,1600,.8);$("sampleAttachmentStatus").textContent=`${technical?`Technický list: ${technical.name} · `:""}fotografia priložená`;renderPreview()}catch(error){$("sampleAttachmentStatus").textContent=error.message}event.target.value=""};qa("#samplePhotoInput,#sampleGalleryInput").forEach(input=>input.onchange=pickPhoto);
   q(".cancel",$("sampleFormMobile")).onclick=closeSheet;
   $("sampleFormMobile").onsubmit=event=>{event.preventDefault();const selected=$("sampleCompanyInput").value,companyId=selected==="__betpres__"?"":selected,submittedBy=selected==="__betpres__"?"BETPRES, s.r.o.":company(selected)?.name||"",date=$("sampleDateInput").value,sequence=Number($("sampleSequenceInput").value)||nextSequence,record={...(existing||{}),id:existing?.id||uid("sample"),projectId:app.projectId,sequence,protocolNumber:existing?.protocolNumber||`VZ-${date.slice(0,4)}-${String(sequence).padStart(3,"0")}`,materialName:$("sampleNameInput").value.trim(),manufacturer:$("sampleManufacturerInput").value.trim(),productType:$("sampleTypeInput").value.trim(),intendedUse:$("sampleUseInput").value.trim(),location:$("sampleLocationInput").value.trim(),companyId,submittedBy,submittedDate:date,status:$("sampleStatusInput").value,technicalSheet:technical,photo,createdAt:existing?.createdAt||isoNow(),updatedAt:isoNow()};upsertRecord("materialSamples",record,"add-sample");closeSheet();route("samples");toast(existing?"Vzorka bola upravená.":"Materiálová vzorka bola uložená.")}
  }})
 }
 ...
 function applyOperation(data,operation){
  normalizeData(data);
  const map={"add-purchase":"purchases","add-defect":"defects","upsert-control-day":"controlDays","add-calendar":"calendarEvents","add-diary":"mobileDiary","add-sample":"materialSamples"},collection=map[operation.kind];
 ...
 async function fileAttachment(file,maxBytes=12*1024*1024){
  if(!file)throw new Error("Vyber súbor.");if(file.size>maxBytes)throw new Error("Súbor je príliš veľký. Maximum je 12 MB.");
  const dataUrl=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||""));reader.onerror=()=>reject(new Error("Súbor sa nepodarilo načítať."));reader.readAsDataURL(file)});
  return{id:uid("file"),name:file.name||"priloha",type:file.type||"application/octet-stream",size:file.size||Math.round(dataUrl.length*.75),dataUrl}
 }
 ...
 async function uploadAttachment(workspaceId,recordId,attachment,kind){
  if(!attachment||attachment.path||!attachment.dataUrl)return attachment;
  const match=attachment.dataUrl.match(/^data:([^;]+);base64,(.+)$/);if(!match)return attachment;
  const extensions={"application/pdf":"pdf","image/png":"png","image/webp":"webp","image/jpeg":"jpg"},extension=extensions[match[1]]||String(attachment.name||"").split(".").pop()?.replace(/[^a-z0-9]/gi,"").toLowerCase()||"bin",path=`${workspaceId}/material-samples/${recordId}/${kind}.${extension}`,encoded=path.split("/").map(encodeURIComponent).join("/"),bytes=Uint8Array.from(atob(match[2]),char=>char.charCodeAt(0));
  const response=await fetch(`${normalizeUrl(app.config.url)}/storage/v1/object/sitedesk-files/${encoded}`,{method:"POST",headers:{apikey:app.config.key,Authorization:`Bearer ${app.session.access_token}`,"Content-Type":match[1],"x-upsert":"true"},body:new Blob([bytes],{type:match[1]})});if(!response.ok)throw new Error(`${kind==="technical"?"Technický list":"Fotografiu vzorky"} sa nepodarilo odoslať.`);return{...attachment,path,dataUrl:undefined}
 }
 async function prepareUploads(data,workspaceId){
  for(const [collection,pathName] of [["defects","defects"],["mobileDiary","diary"],["purchases","materials"]])for(const record of data[collection]||[]){if(!Array.isArray(record.photos))continue;record.photos=await Promise.all(record.photos.map(photo=>uploadPhoto(workspaceId,pathName,record.id,photo)))}
  for(const record of data.materialSamples||[]){record.technicalSheet=await uploadAttachment(workspaceId,record.id,record.technicalSheet,"technical");record.photo=await uploadAttachment(workspaceId,record.id,record.photo,"photo")}
  return data
 }
 ...
 function openQuickOpen(){
  const commands=[
   ...
   {icon:"◇",title:"Materiálové vzorky",detail:"Technické listy a schvaľovanie investorom",route:"samples",keywords:"vzorkovanie protokol materiál"},
   ...
   {icon:"＋",title:"Nová materiálová vzorka",detail:"Priložiť technický list a fotografiu",action:"sample",keywords:"pridať vzorku"},
 ...
 function wireEvents(){
  ...
  $("materialSearch").oninput=renderMaterials;$("defectSearch").oninput=renderDefects;$("sampleSearch").oninput=renderSamples;$("diarySearch").oninput=renderDiary;
