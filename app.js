(() => {
 ...
 function renderDefects(){
  ...
  $("defectsList").innerHTML=items.length?items.map(item=>`<article class="list-item" data-defect-id="${esc(item.id)}"><span class="status-dot ${isDefectDone(item)?"done":""}"></span><div class="item-main"><strong class="${/vysok/i.test(item.severity||"")?"severity-high":""}">${esc(item.number?`${item.number} · `:"")}${esc(item.description||"Vada bez popisu")}</strong><span>${esc(item.location||"Miesto neuvedené")} · ${esc(company(item.companyId)?.name||"Bez firmy")}</span></div><div class="item-side"><strong>${esc(item.status||"Otvorená")}</strong><small>termín ${formatDate(item.dueDate)}</small></div></article>`).join(""):`<div class="empty-state">V tomto filtri nie sú žiadne vady ani nedorobky.</div>`
 }
 ...
 function openMaterialForm(){
  ...
  showSheet({eyebrow:"PASPORT SKLADU",title:"Nový dodací list",html:`<form id="materialForm" class="sheet-form"><div class="photo-picker"><label>📷 Odfotiť a rozpoznať<input id="materialCamera" type="file" accept="image/*" capture="environment"></label><label>▧ Vybrať fotografiu<input id="materialPhoto" type="file" accept="image/*"></label></div><p id="ocrStatus" class="form-note">OCR pracuje priamo v telefóne. Po rozpoznaní skontroluj údaje.</p><div class="form-split"><label>Dátum<input id="materialDate" type="date" required value="${todayISO()}"></label><label>P. č.<input id="materialSequence" inputmode="numeric" value="${esc(nextSequence)}"></label></div><label>Dodávateľ<input id="materialSupplierInput" required placeholder="napr. DEK"></label><label>Číslo dodacieho listu<input id="materialDocumentNo" placeholder="Číslo dokladu"></label><label>Číslo faktúry<input id="materialInvoiceNo" placeholder="Nechaj prázdne, kým nie je faktúra"></label><label>Materiál<input id="materialName" required placeholder="napr. vata / spotrebný materiál"></label><label>Orientačná cena<input id="materialPrice" inputmode="decimal" placeholder="0,00 €"></label><div class="sheet-actions"><button type="button" class="cancel">Zrušiť</button><button class="submit" type="submit">Uložiť dodací list</button></div></form>`,onReady:()=>{
   let photo=null,ocrText="";
   qa("#materialCamera,#materialPhoto").forEach(input=>input.onchange=async()=>{const file=input.files?.[0];if(!file)return;try{photo=await imageData(file,1800,.82);const status=$("ocrStatus");status.textContent="Rozpoznávam dodací list… 0 %";const result=await recognizeDelivery(photo.dataUrl,progress=>status.textContent=`Rozpoznávam dodací list… ${Math.round(progress*100)} %`);ocrText=result.text||"";const parsed=parseDeliveryOcr(ocrText);$("materialDate").value=parsed.date;$("materialSupplierInput").value=parsed.supplier;$("materialDocumentNo").value=parsed.documentNo;$("materialName").value=parsed.material;$("materialPrice").value=parsed.estimatedPrice;status.textContent=`Rozpoznanie dokončené · istota ${Math.round(result.confidence||0)} % · údaje skontroluj.`}catch(error){$("ocrStatus").textContent=`OCR sa nepodarilo: ${error.message}. Údaje môžeš zapísať ručne.`}});
   q(".cancel",$("materialForm")).onclick=closeSheet;
   $("materialForm").onsubmit=event=>{event.preventDefault();const record={id:uid("pd"),projectId:app.projectId,date:$("materialDate").value,supplier:$("materialSupplierInput").value.trim(),documentNo:$("materialDocumentNo").value.trim(),invoiceNo:$("materialInvoiceNo").value.trim(),material:$("materialName").value.trim(),estimatedPrice:$("materialPrice").value.trim(),sequence:$("materialSequence").value.trim()||nextSequence,source:photo?"OCR mobil":"Mobil",ocrText,photos:photo?[photo]:[],createdAt:isoNow()};addRecord("purchases",record,"add-purchase");closeSheet();route("materials");toast("Dodací list bol uložený.")}
  }})
 }

 function openDefectForm(){
  showSheet({eyebrow:"KONTROLA KVALITY",title:"Nová vada / nedorobok",html:`<form id="defectFormMobile" class="sheet-form"><label>Popis vady<textarea id="defectDescriptionInput" required placeholder="Čo treba opraviť alebo dokončiť?"></textarea></label><div class="form-split"><label>Miesto<input id="defectLocationInput" placeholder="napr. 6. NP, byt 34"></label><label>Termín<input id="defectDueInput" type="date"></label></div><label>Zodpovedná firma<select id="defectCompanyInput">${companyOptions()}</select></label><div class="form-split"><label>Závažnosť<select id="defectSeverityInput"><option>Nízka</option><option selected>Stredná</option><option>Vysoká</option></select></label><label>Stav<select id="defectStatusInput"><option>Otvorená</option><option>Prebieha</option><option>Odoslaná firme</option><option>Odstránená</option></select></label></div><label class="connection-import">📷 Pridať fotografie<input id="defectPhotosInput" type="file" accept="image/*" capture="environment" multiple></label><p id="defectPhotoStatus" class="form-note">Môžeš pridať najviac 6 fotografií.</p><div class="sheet-actions"><button type="button" class="cancel">Zrušiť</button><button class="submit" type="submit">Uložiť vadu</button></div></form>`,onReady:()=>{
   let photos=[];$("defectPhotosInput").onchange=async event=>{photos=[];for(const file of [...event.target.files].slice(0,6))photos.push(await imageData(file,1400,.72));$("defectPhotoStatus").textContent=`Pripravené fotografie: ${photos.length}`};
   q(".cancel",$("defectFormMobile")).onclick=closeSheet;
   $("defectFormMobile").onsubmit=event=>{event.preventDefault();const number=String(Math.max(0,...currentProjectItems("defects").map(item=>Number(String(item.number||"").replace(/\D/g,""))||0))+1),status=$("defectStatusInput").value,record={id:uid("defect"),projectId:app.projectId,companyId:$("defectCompanyInput").value,number,location:$("defectLocationInput").value.trim(),description:$("defectDescriptionInput").value.trim(),dueDate:$("defectDueInput").value,severity:$("defectSeverityInput").value,status,responsible:"",photos,createdAt:isoNow(),updatedAt:isoNow(),sentAt:status==="Odoslaná firme"?isoNow():""};addRecord("defects",record,"add-defect");closeSheet();route("defects");toast("Vada bola uložená.")}
  }})
 }

 function openDiaryForm(){
  showSheet({eyebrow:"MÔJ STAVEBNÝ DENNÍK",title:"Nový denný zápis",html:`<form id="diaryFormMobile" class="sheet-form"><label>Názov<input id="diaryTitleInput" required placeholder="Čo si si všimol?"></label><label>Poznámka<textarea id="diaryTextInput" required placeholder="Zapíš detail, otázku alebo úlohu na neskôr."></textarea></label><div class="form-split"><label>Miesto<input id="diaryPlaceInput" placeholder="napr. strecha"></label><label>Termín<input id="diaryDueInput" type="date"></label></div><label>Priorita<select id="diaryPriorityInput"><option>Nízka</option><option selected>Stredná</option><option>Vysoká</option></select></label><label class="connection-import">📷 Pridať fotografie<input id="diaryPhotosInput" type="file" accept="image/*" capture="environment" multiple></label><p id="diaryPhotoStatus" class="form-note">Fotografie sú dostupné aj offline a odošlú sa pri synchronizácii.</p><div class="sheet-actions"><button type="button" class="cancel">Zrušiť</button><button class="submit" type="submit">Uložiť zápis</button></div></form>`,onReady:()=>{
   let photos=[];$("diaryPhotosInput").onchange=async event=>{photos=[];for(const file of [...event.target.files].slice(0,6))photos.push(await imageData(file,1400,.72));$("diaryPhotoStatus").textContent=`Pripravené fotografie: ${photos.length}`};
 ...
 async function recognizeDelivery(dataUrl,onProgress=()=>{}){
  ...
  if(!app.ocrWorker)app.ocrWorker=window.Tesseract.createWorker("slk",1,{workerPath:"https://cdn.jsdelivr.net/npm/tesseract.js@7.0.0/dist/worker.min.js",langPath:"https://cdn.jsdelivr.net/npm/@tesseract.js-data/slk@1.0.0/4.0.0",corePath:"https://cdn.jsdelivr.net/npm/tesseract.js-core@7.0.0/tesseract-core-lstm.wasm.js",logger:message=>{if(message.status==="recognizing text")onProgress(message.progress||0)}});
